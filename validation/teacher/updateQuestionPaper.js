...
module.exports = data => {
    ...
    if(data.question_num){
        ...
        if(Validator.isEmpty(data.correct_option)) errors.correct_option = "This field is required"
        
        if(Validator.isEmpty(data.marks_alloted)) errors.marks_alloted = "This field is required"
        
        if(data.options.split(',').length <= 1) errors.options = "More than one option is required"
        ...
        if(Validator.isEmpty(data.total_marks)) errors.total_marks = "This field is required"
    }
